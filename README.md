...
Notes:
1) React-redux -- important modules are provider and connect
2) Provider passes the store to the underlying components
3) connect is responsible for exposing relevant state slice and props to the component from the store
4) in line string substitution needs to use ` instead of '
5) ES6 short hand property names. If the property name and the object name matches, dont need to specify both. For example {courses:courses} is not needed and only {courses} would suffice
6) Not wiring up the mapDispatchToProps will inject the dispatch object to the props object of the container.